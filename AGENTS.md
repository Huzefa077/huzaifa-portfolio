...
- Keep one long-lived branch: `main`
- Make small conventional commits directly on `main` (e.g. `feat:`, `fix:`, `refactor:`)
- Push `main` after verified commits; pushes automatically deploy through GitHub Actions
- Never force-push or rewrite `main` history without explicit user approval

## Tech Stack

Next.js 16 (App Router) · React 19 · TypeScript · Tailwind CSS v4 · Biome
...
- **Compiler**: both `npm run dev` and `npm run build` intentionally use `--webpack`. Turbopack repeatedly panics during development with `Next.js package not found`, and its production build tracer stalls on the build-time public-image header reader. Do not restore Turbopack until both failures are verified fixed
- **Canonical/export URLs**: When generating absolute URLs for metadata, sitemap, or schema, match `trailingSlash: true` output (`/about/`, `/blog/post-slug/`) instead of non-canonical no-slash variants; file-like routes such as `/sitemap.xml` stay file-like
...
- **Blog posts**: Markdown files in `content/blog/` with frontmatter (title, date, description); slug derived from filename
- **Blog post headings**: the post page renders the frontmatter title as the single `<h1>`, so Markdown bodies start with prose or an `<h2>` section and must not repeat the title
- **Blog sharing**: use the shared `ShareButton` client component with an absolute production URL. It opens the native device share sheet (including installed social apps) and falls back to copying the URL
- **Blog page**: Add external links in `src/data/blog.ts` and keep dated entries sorted newest first; local posts still live in `content/blog/`
...
When making a substantial change, audit this file and make small, targeted improvements based on your learnings—new patterns discovered, outdated references, or missing guidance that would have helped.
